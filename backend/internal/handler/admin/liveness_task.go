...
func (t *livenessTask) snapshot() *livenessTask {
	t.mu.RLock()
	defer t.mu.RUnlock()
	copy := *t
	copy.Recent = append([]livenessTaskResult(nil), t.Recent...)
	copy.failedAccountIDs = nil
	return &copy
}
...
func (t *livenessTask) record(result livenessTaskResult, updateFailed bool) {
	...
	if result.Status == "alive" {
		...
		if updateFailed { t.UpdateFailed++ }
	}
	t.Recent = append([]livenessTaskResult{result}, t.Recent...)
	if len(t.Recent) > livenessTaskRecentLimit { t.Recent = t.Recent[:livenessTaskRecentLimit] }
}
...
func newLivenessTaskStore() *livenessTaskStore { return &livenessTaskStore{tasks: make(map[string]*livenessTask)} }

func (s *livenessTaskStore) create(total int) *livenessTask {
	task := &livenessTask{ID: fmt.Sprintf("liveness-%d", time.Now().UnixNano()), State: "running", CreatedAt: time.Now(), Total: total}
	s.mu.Lock()
...
func (h *AccountHandler) runLivenessTask(task *livenessTask, accountIDs []int64) {
	...
	for _, accountID := range accountIDs {
		...
		g.Go(func() error {
			...
			if testErr != nil { errorMessage = testErr.Error() } else if result != nil && result.ErrorMessage != "" { errorMessage = result.ErrorMessage }
			if len(errorMessage) > 1000 { errorMessage = errorMessage[:1000] }
...
func (h *AccountHandler) GetBatchLivenessTask(c *gin.Context) {
	...
	if !ok { response.NotFound(c, "Liveness task not found"); return }
...
func (h *AccountHandler) DeleteBatchLivenessFailed(c *gin.Context) {
	...
	if !ok { response.NotFound(c, "Liveness task not found"); return }
	task.mu.RLock()
	if task.State != "completed" { task.mu.RUnlock(); response.BadRequest(c, "Liveness task is still running"); return }
	ids := append([]int64(nil), task.failedAccountIDs...)
	task.mu.RUnlock()
	success, failed := 0, 0
	deleted := make(map[int64]struct{}, len(ids))
	for _, accountID := range ids {
		if err := h.adminService.DeleteAccount(c.Request.Context(), accountID); err != nil { failed++; continue }
		success++
	...
	for _, id := range task.failedAccountIDs { if _, ok := deleted[id]; !ok { remaining = append(remaining, id) } }
